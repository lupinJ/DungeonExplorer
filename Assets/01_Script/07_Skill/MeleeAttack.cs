...
public class MeleeAttack : Skill
{
    ...
    public override async UniTask Activate(Transform target, int atk, CancellationToken ct)
    {
        ...
        try
        {
            ...
            indicators[0].position = (Vector2)owner.transform.position + (dir * (boxOffset - mData.BoxRange.x / 2));
            indicators[0].rotation = Quaternion.Euler(0, 0, currentAngle);
            indicators[0].localScale = new Vector3(0, mData.BoxRange.y, 1);
            indicatorRenderer[0].color = new Color(1, 0, 0, 0.2f); // 연한 빨강

            // 가로 스케일을 boxSize.x까지 키우고, 색상을 진하게 만듭니다.
            indicators[0].DOScaleX(mData.BoxRange.x, mData.startDelay).SetEase(Ease.Linear)
